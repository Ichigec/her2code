...
**When activated via `/agent plan2`, treat the user's message as the task description and IMMEDIATELY begin Phase 1.** The user selected `/agent plan2` to get work done, not to chat. Full cycle is the DEFAULT.
...
**Before Phase 1, read:**
1. `read_file("$HERMES_HOME/AGENTS.md")` — project conventions, build commands, environment, pitfalls
2. `read_file("$HERMES_HOME/auditor_memory.md")` — cross-cycle patterns, agent performance trends
...
cp $HERMES_HOME/AGENTS.md /home/user/dev/codemes/$PID/AGENTS.md 2>/dev/null || true
```
...
python3 $HERMES_HOME/scripts/capability_gate.py \
  --task "$TASK" \
...
| **5.5** | **Pre-Flight Gate** | Orchestrator | `python3 $HERMES_HOME/scripts/orchestrator_gate.py`. 7 checks. FAIL = BLOCKED. |
...
|---|-------|----------------|----------------|----------|
| 0 | Project Bootstrap + AFlow spawn | `/agent plan2` activated; CWD is project root | `/home/user/dev/codemes/{pid}/` exists with AGENTS.md + structure.md. AFlow Orchestrator spawned. | `rm -rf /home/user/dev/codemes/{pid}/` |
...
delegate_task(
  ...
  context="""
    ...
    Agent file: $HERMES_HOME/agents/aflow-orchestrator.md — загрузи через read_file для полных инструкций.
  """,
  agent="aflow-orchestrator",
  model="deepseek-v4-pro", provider="custom:local", role="orchestrator"
)
```
...
```python
# Auditor — agent file: $HERMES_HOME/agents/auditor.md
delegate_task(
  ...
  context="""
    Cycle ID: {pid}
    Task: {task_description}
    Твой agent-файл: $HERMES_HOME/agents/auditor.md
  ...
  model="deepseek-v4-pro", provider="custom:local", role="leaf"
)

# Critic — agent file: $HERMES_HOME/agents/critic.md
delegate_task(
  goal="Ищи лишнее, мешающее, причины усложнения. На КАЖДОМ checkpoint ПИШИ в Neo4j: CREATE (:CriticFinding).",
  context="""
    Cycle ID: {pid}
    Agent file: $HERMES_HOME/agents/critic.md
    Neo4j: neo4j:<YOUR_NEO4J_PASSWORD>@localhost:7474
    ВАЖНО: ПИШИ в Neo4j после каждого checkpoint.
  """,
  toolsets=["file_ro", "search_files", "session_search", "terminal"],
...
# Idea Generator — agent file: $HERMES_HOME/agents/idea-generator.md
delegate_task(
  goal="Лови неслышанные идеи, ищи connections, предлагай оптимизации. ПИШИ в Neo4j: CREATE (:Idea) + CREATE (:Mutation).",
  context="""
    Cycle ID: {pid}
    Agent file: $HERMES_HOME/agents/idea-generator.md
    Neo4j: neo4j:<YOUR_NEO4J_PASSWORD>@localhost:7474
    ВАЖНО: ПИШИ в Neo4j после каждого checkpoint.
  """,
...
# Knowledge Curator — agent file: $HERMES_HOME/agents/knowledge-curator.md
delegate_task(
  goal="Извлекай entities из каждого артефакта, сохраняй в Neo4j: MERGE (:KnowledgeEntity).",
  context="""
    Cycle ID: {pid}
    Agent file: $HERMES_HOME/agents/knowledge-curator.md
    Neo4j: neo4j:<YOUR_NEO4J_PASSWORD>@localhost:7474
    ВАЖНО: ПИШИ в Neo4j после каждого checkpoint.
  """,
...
**Structured output:** Phase 3 outputs `docs/research/<slug>.json` (structured, PRIMARY) + `docs/research/<slug>.md` (auto-generated view). JSON is the single source of truth. See schema: `$HERMES_HOME/schemas/research-output-v1.json`.
...
delegate_task(
  ...
  model="deepseek-v4-pro", provider="custom:local"
)
```
...
**GATE B:** `python3 $HERMES_HOME/scripts/research_quality_gate.py --artifact docs/research/<slug>.json`
...
delegate_task(
  ...
  Output MUST follow schema: $HERMES_HOME/schemas/research-output-v1.json (tiered: structured core + conditional + unstructured_notes + must_see).
  After writing JSON, run: python3 $HERMES_HOME/scripts/research_json_to_md.py --input docs/research/<slug>.json --output docs/research/<slug>.md""",
  agent="deep-plan-researcher",
  model="deepseek-v4-pro", provider="custom:local"
)
```

**GATE C:** `python3 $HERMES_HOME/scripts/research_completeness_gate.py --artifact docs/research/<slug>.json`
...
**GATE D:** `python3 $HERMES_HOME/scripts/citation_enforcement_gate.py --artifact docs/research/<slug>.json`
...
```bash
python3 $HERMES_HOME/scripts/orchestrator_gate.py --json
```
...
### Phase 6 — Progressive Development Pipeline (delegated to Tech Lead v3)

**Tech Lead v3 = sub-orchestrator for Phase 6.** Оркестратор делегирует весь Phase 6 Tech Lead'у:

```python
delegate_task(
  goal="Execute Phase 6 Dev Pipeline. Plan: <plan_path>. "
       "Ownership: <ownership_path>. DAG state: <dag_state_path>. "
       "Codebase deps: <deps_summary>. "
       "PID: <pid>. Execute all StandardWork tasks, manage escalation, "
       "run Review Swarm, merge. Collect feedback after EACH SW (6.5a). "
       "Run pattern detection after every 2nd SW (6.5b). "
       "Apply loop guards (6.5d). Update dag-state.json. Return Phase 6 Report.",
  context="<plan_path contents summary> + <ownership matrix> + <dag_state_path> + <codebase deps>",
  toolsets=["delegation", "terminal", "file", "search_files", "session_search", "skills"],
  model="glm-5.2", provider="custom:local",
  role="orchestrator"  # ← sub-orchestrator, can spawn developers
)
```

**Tech Lead v3 управляет:**
1. Spawning developers (Skeptic→Pragmatic→Creative→Maverick escalation)
2. Review Swarm (5 reviewers per PASS)
3. Sequential dependencies
4. **Feedback Loop Closure** — collect feedback after EACH SW, pattern detection, plan revision
5. **DAG state management** — update dag-state.json at every event, apply loop guards
6. Final merge
7. Phase 6 Report (with DAG version, revisions, patterns, budget) → оркестратор

**Fallback:** при timeout/error оркестратор выполняет Phase 6 напрямую (старая модель).

---

### Phase 6 (legacy) — Progressive Development Pipeline (orchestrator direct)

**Note:** В legacy mode оркестратор выполняет Phase 6 напрямую. Feedback loop (6.5a-6.5d) и DAG updates ОБЯЗАТЕЛЬНЫ даже в этом режиме — читай и обновляй dag-state.json после каждого SW.

**DAG-driven execution.** DAG state lives in `.hermes/plans/<ts>-<slug>-dag.json` (created by Tech Lead in Phase 5).
...
- ACON feedback: if developer needed a filtered finding → `$HERMES_HOME/plans/<cycle>-filter-rules.json` updated for next cycle
...
- **Style Reviewer** — `$HERMES_HOME/agents/review/style-reviewer.md`
- **Bug Reviewer** — `$HERMES_HOME/agents/review/bug-reviewer.md`
- **Security Reviewer** — `$HERMES_HOME/agents/review/security-reviewer.md`
- **Perf Reviewer** — `$HERMES_HOME/agents/review/perf-reviewer.md`
- **Convention Reviewer** — `$HERMES_HOME/agents/review/convention-reviewer.md`

Each reviewer returns confidence ≥0.7. Aggregate feedback → Skeptic verifies all addressed.

**Agent files:** `$HERMES_HOME/agents/dev/dev-skeptic.md`, `dev-pragmatic.md`, `dev-creative.md`, `dev-maverick.md`.
  ...
  tasks=[
    {
      ...
      context: "Фаза: {phase_name}. Артефакт: {path}. Цикл: {pid}. Agent file: $HERMES_HOME/agents/auditor.md. Neo4j: neo4j:<YOUR_NEO4J_PASSWORD>@localhost:7474. SDB: VERIFY → COMMIT в Neo4j.",
      toolsets: ["file_ro", "search_files", "session_search", "terminal"],
      model: "deepseek-v4-pro", provider: "custom:local"
    },
    {
      goal: "Critic checkpoint Phase {N}: ищи лишнее и причины усложнения. ЗАПИШИ в Neo4j: CREATE (:CriticFinding) + связь [:FOUND_IN]->(:Phase).",
      context: "Фаза: {phase_name}. Артефакт: {path}. Цикл: {pid}. Agent file: $HERMES_HOME/agents/critic.md. Neo4j: neo4j:<YOUR_NEO4J_PASSWORD>@localhost:7474. SDB: VERIFY → COMMIT в Neo4j.",
      toolsets: ["file_ro", "search_files", "session_search", "terminal"],
      model: "deepseek-v4-pro", provider: "custom:local"
    },
    {
      goal: "Idea Generator checkpoint Phase {N}: лови идеи и connections. ЗАПИШИ в Neo4j: CREATE (:Idea) + CREATE (:Mutation).",
      context: "Фаза: {phase_name}. Артефакт: {path}. Цикл: {pid}. Agent file: $HERMES_HOME/agents/idea-generator.md. Neo4j: neo4j:<YOUR_NEO4J_PASSWORD>@localhost:7474. SDB: VERIFY → COMMIT в Neo4j.",
      toolsets: ["file_ro", "search_files", "session_search", "terminal", "skills", "memory"],
      model: "deepseek-v4-pro", provider: "custom:local"
    },
    {
      goal: "Knowledge Curator checkpoint Phase {N}: извлеки entities, сохрани в Neo4j: MERGE (:KnowledgeEntity).",
      context: "Фаза: {phase_name}. Артефакт: {path}. Цикл: {pid}. Agent file: $HERMES_HOME/agents/knowledge-curator.md. Neo4j: neo4j:<YOUR_NEO4J_PASSWORD>@localhost:7474. SDB: VERIFY → COMMIT в Neo4j.",
      toolsets: ["file_ro", "search_files", "session_search", "skills", "memory", "terminal"],
      model: "deepseek-v4-pro", provider: "custom:local"
    }
  ]
...
**registry.json** at `$HERMES_HOME/agents/registry.json` defines all 29 agents with their models, toolsets, and permissions. Orchestrator reads registry for delegation parameters.

**Перед каждым циклом оркестратор обновляет registry:**
```bash
python3 $HERMES_HOME/scripts/agent_registry.py
```
  ...
  tasks=[
    {
      ...
      context: "Cycle: {pid}. Agent file: $HERMES_HOME/agents/auditor.md. Checkpoints: .observations/cycle-{pid}/auditor-phase-*.md.",
...
Auditor ДОПИСЫВАЕТ в `$HERMES_HOME/auditor_memory.md` результаты цикла. Формат:
...
python3 $HERMES_HOME/scripts/capability_gate.py --task "$TASK" --json
# → GateVerdict {Go|Kill|Hold|Recycle}

# Post-plan fabrication scan — after Phase 5
python3 $HERMES_HOME/scripts/capability_gate.py --post-plan .hermes/plans/<slug>.md
