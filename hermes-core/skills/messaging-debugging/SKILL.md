...
Returns channel info if bot has access. The `id` field is the numeric chat_id (negative for channels/groups, e.g. `<YOUR_TELEGRAM_CHAT_ID>`).
...
```
target: "telegram:<chat_id>"   # e.g. "telegram:<YOUR_TELEGRAM_CHAT_ID>"
```
