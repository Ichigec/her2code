...
- When choosing between structural alternatives (monolith vs services, sync
  vs async, etc.).
- **For agent framework designs** (orchestrators, tool registries, gate/pipeline
  systems), see `references/agent-infrastructure-architecture.md` for the
  extended pattern with interface contracts, data flows, and rollout planning.
...
### Agent infrastructure exception
- **For agent frameworks and tool registries, component contracts ARE code-level interfaces.** ABC signatures, JSON schemas, and `@dataclass` field definitions define the wire protocol between components — they belong in architecture. See `references/agent-infrastructure-architecture.md` for the full pattern and when it applies. The rule "no pseudocode" means no function bodies, business logic, or procedural algorithms — not interface definitions.
