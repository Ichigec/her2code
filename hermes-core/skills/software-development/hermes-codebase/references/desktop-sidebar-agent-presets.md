...
- **The wiring example above uses the OLD `/agent` command-insert approach.** As of July 2026, agent presets use `agents.activate` RPC (via `tui_gateway/server.py`) and `$activeAgentPresetId` nanostore. The full lifecycle (persistence, re-apply on reconnect, pitfall about empty `agent_id` clearing the indicator) is documented in `hermes-desktop-extension` → `references/agent-preset-buttons.md` — read that before working on preset-related features.
