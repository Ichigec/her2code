...
- **`provider:` is NOT a valid frontmatter field.** `AgentDef` has no `provider`
  field — `from_mapping()` in `agent/agents.py:350` reads `model` but silently
  ignores `provider`. `apply_agent()` calls `switch_model(new_model=...)` without
  `new_provider`, so even the model switch silently fails (exception swallowed).
  The agent always runs on the session default model+provider regardless of what
  `provider:` says in the frontmatter. Provider routing for subagents works only
  through `delegate_task(provider="...")` in the orchestrator's instruction body.
  Writing `provider: custom:kimi` in frontmatter is cosmetic — it does nothing.
