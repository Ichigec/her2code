...
- **[advanced-usage.md](references/advanced-usage.md)** — speculative decoding (EAGLE3, MTP, classic draft, n-gram), batched inference, grammar-constrained generation, LoRA, multi-GPU, custom builds, benchmark scripts
...
- **[diffusion-gemma.md](references/diffusion-gemma.md)** — DiffusionGemma block-diffusion architecture: PR #24423 fork, build instructions, CLI flags, non-HTTP-server caveats
- **[diffusion-gemma-api-wrapper.md](references/diffusion-gemma-api-wrapper.md)** — Python FastAPI wrapper: exposes OpenAI-compatible API around llama-diffusion-cli; LiteLLM integration; env vars; startup; pitfalls
- **[litellm-ops.md](references/litellm-ops.md)** — LiteLLM proxy operational issues: 21 GB memory leak diagnosis, fix via worker recycling, inspection commands
