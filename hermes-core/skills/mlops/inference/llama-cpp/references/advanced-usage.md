...
### EAGLE3 Draft Model (PR #18039, up to 5× speedup)

EAGLE3 uses a separately trained lightweight draft model (~0.4B params) that
predicts future tokens using the target model's hidden states. See the
`speculative-decoding` skill for training workflow.
...
llama-server -m target-model.gguf \
    ...
    --draft 4  # Number of draft tokens before verification
```

Draft models can be trained with `vllm-project/speculators` framework and
converted to GGUF via `convert_hf_to_gguf.py`.

### MTP — Multi-Token Prediction (PR #22673, merged May 2026)

Native speculative decoding using built-in MTP heads. No separate draft model
needed. Supported on Qwen3.5/3.6, DeepSeek-V3, Gemma 4.

```bash
# MTP speculative decoding (model must have MTP head)
llama-server -m qwen3.6-35b-mtp.gguf --mtp
```

MTP heads can be grafted from a base checkpoint onto fine-tuned/abliterated
models. See `speculative-decoding` skill → MTP Grafting section.

### Classic Draft Model Approach
...
**Note:** MoE models (e.g., Qwen3-30B-A3B) show no net speedup with classic
draft model approach in llama.cpp — expert routing overhead eats the gain.
Use vLLM or SGLang for MoE speculative decoding. See `speculative-decoding`
skill → MoE-Specific Considerations.

### Self-Speculative Decoding (n-gram / prompt-lookup)
