...
- For MoE models, see the `local-model-serving` skill for APEX mixed-precision quantization (beats Q8_0 at half the size) and agentic calibration corpus design (function calling + chat + code in model's native chat format)

## APEX Quantization (MoE models)

For Mixture-of-Experts models, APEX (from LocalAI) applies per-tensor-type and per-layer precision gradients that beat Q8_0 perplexity at half the size. Uses stock llama.cpp — no patches required.

```bash
# I-variant (with diverse imatrix, recommended for agentic models):
cd /path/to/apex-quant
NUM_LAYERS=40 LLAMA_CPP_DIR=/path/to/llama.cpp/build/bin \
  ./scripts/quantize.sh --profile i-quality --imatrix model.imatrix.gguf model-f16.gguf model-apex.gguf
```

Profiles: `quality`, `i-quality`, `balanced`, `i-balanced`, `compact`, `i-compact`, `mini`.
I-variants use diverse calibration (chat, code, reasoning, tool-calling — no Wikipedia).

**Full APEX pipeline, calibration corpus design, benchmark results, and OOM mitigation strategies are in the `local-model-serving` skill** — including reference files for agentic calibration corpus building, SuperQwen benchmark results, and OOM killer forensics during quantization.
