...
### TypeError: target_model_dir / fp8_as_q8 unexpected kwarg

**Error**: `TypeError: ModelBase.__init__() got an unexpected keyword argument 'target_model_dir'`

**Cause**: Certain llama.cpp commits have `convert_hf_to_gguf.py` passing `target_model_dir` and `fp8_as_q8` kwargs to `ModelBase.__init__()`, but `conversion/base.py` doesn't accept them (API drift between the script and the class).

**Fix**: Remove those two kwargs from the constructor call in `convert_hf_to_gguf.py`:

```bash
cp convert_hf_to_gguf.py convert_hf_to_gguf.py.bak
sed -i '/target_model_dir=/d' convert_hf_to_gguf.py
sed -i '/fp8_as_q8=/d' convert_hf_to_gguf.py
```

If `convert_hf_to_gguf.py` is root-owned (common when llama.cpp was installed via Docker), copy `conversion/` to a writable location and use `PYTHONPATH`:

```bash
cp -r conversion /tmp/llama_conversion
PYTHONPATH=/tmp/llama_conversion python3 convert_hf_to_gguf.py <model_dir> --outtype f16 --outfile model.gguf
```

Or if the main script IS writable (just `conversion/` is root-owned), edit the script directly — it only needs `fuse_gate_up_exps=` to remain (the other two kwargs are non-essential).

**Dependency setup for conversion** (if starting from a clean system):

```bash
cd llama.cpp
python3 -m venv .venv
source .venv/bin/activate
# CUDA build of torch for Jetson/DGX (aarch64):
...
**Error**: `llama_init_from_model: V cache quantization requires flash_attn` → `main: error: failed to create context with model`

**Cause**: Using any non-f16 KV cache type (`-ctk q8_0 -ctv q8_0`, `type_k=2`, etc.) REQUIRES flash attention to be enabled. This is a hard requirement, not an optimization. Without flash attention, the quantized KV cache path is not implemented and context creation fails.

**Fix for llama-bench / llama-cli / llama-server (CLI flags)**:
```bash
# MUST include -fa 1 when using -ctk/-ctv with anything other than f16
./llama-bench -m model.gguf -ngl 99 -ctk q8_0 -ctv q8_0 -fa 1 -p 128 -n 64
./llama-server -m model.gguf -ngl 99 -ctk q8_0 -ctv q8_0 --flash-attn on -c 262144
```
...
llm = Llama(
    ...
    n_gpu_layers=35,
    flash_attn=True,     # REQUIRED when type_k/type_v != f16
)
```

**Pitfall**: `llama-bench` uses single-dash flags (`-mmp`, `-fa`, `-ctk`) while `llama-server` uses long flags (`--mmap`, `--flash-attn`, `--cache-type-k`). Mixing them causes confusing parse errors — always check `--help` for the specific binary.
...
4. **Quantize KV cache** (REQUIRES flash attention — see section above):
```python
llm = Llama(
    model_path="model.gguf",
    type_k=2,            # Q4_0 for K cache
    type_v=2,            # Q4_0 for V cache
    n_gpu_layers=35,
    flash_attn=True,      # MANDATORY with quantized KV cache
)
```
