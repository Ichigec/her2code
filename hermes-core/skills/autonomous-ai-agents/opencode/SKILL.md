...
For Pavel's opencode+ setup (`/home/user/cursor/opencode+/configs/opencode.litellm-dual.json`), see `references/opencode-agent-configuration.md` for the full agent comparison table, the `general` agent deep-dive, and the Hermes→OpenCode prompt porting analysis.

## Hermes vs OpenCode: Architecture Comparison

OpenCode uses an **async HTTP session model** (Node.js event loop, no per-subagent
threads). Hermes uses **in-process ThreadPoolExecutor threads** (Python, synchronous
agent loop). For the full comparison — resource overhead, blocking behavior, what
Hermes already has that could bridge the gap, and what blocks HTTP delegation today —
see the `hermes-codebase` skill's `references/subagent-process-model.md`.

Key difference: OpenCode's `POST /session/:id/prompt_async` enables non-blocking
subagent invocation. Hermes `delegate_task` blocks the parent until all children
finish. Hermes already has an HTTP API server (`:8642`) with `_create_agent()` and
`/v1/runs` that could serve as an async delegation backend, but it's not wired to
`delegate_task`.
...
OpenCode+ uses a LiteLLM proxy (`http://127.0.0.1:4000/v1`) to front LM Studio, llama.cpp, LocalAI, vLLM, DeepSeek, and OpenAI. If a model is visible in `/v1/models` but chat requests fail with authentication errors, see `references/opencode-plus-litellm-proxy.md` for the DeepSeek provider-prefix/env-file pitfall and the canonical restart procedure.
...
- **OpenCode+ LiteLLM proxy auth failures are usually a provider/env mismatch, not a bad key.** DeepSeek must use the native `deepseek/` provider in `config.yaml`; `openai/deepseek-*` sends `OPENAI_API_KEY` to DeepSeek and produces a 401. Also verify the key exists in the `.env` file that `compose.phoenix.yml` is actually started with (`/home/user/cursor/.env`). See `references/opencode-plus-litellm-proxy.md`.
