...
8. **Rollback doc**: record exact repo files, config backup path, and `git restore`/`cp -p` rollback commands. Do not use `git restore .` in User's Hermes repo because unrelated local work is often present.
...
- Keep rollback scoped to touched files; User's Hermes checkout commonly has unrelated modified/untracked files.
