...
| Two variants (public / pers) | Public: no memory/personal data. Pers: full User config |
| Date-based versioning | 2026.06.14 — simpler than semver for a config distro |
| MIT license | Simple, permissive |
| Russian onboarding | User's requirement — all first-run instructions in Russian |
| merge strategy (not overwrite) | User's requirement — upgrade must supplement, not replace |
