...
Full user docs: https://hermes-agent.nousresearch.com/docs/

**Android voice chat development:** `skill_view(name="hermes-agent", file_path="references/android-voice-chat.md")` — architecture, STT/TTS patterns, tunneling, critical bugs, code examples.

**Cellular/VPS tunnel setup:** `skill_view(name="hermes-agent", file_path="references/cellular-vps-tunnel.md")` — SSH reverse tunnel, unified proxy pattern, LiteLLM routing, stale session cleanup, step_start handling.
...
1. Open @BotFather in Telegram → `/newbot` → provide name + username
2. Get the API Token (format `123456789:ABCdef...`)
3. Store in `.env`: `TELEGRAM_BOT_TOKEN=***`
4. Or set in `config.yaml` under `platforms.telegram.token`
5. `hermes gateway setup telegram` or `hermes gateway run`
6. DM your bot → `/approve` to authorize (or auto-approve)
7. Gateway must stay running (use `hermes gateway install` + `start` for persistence)

#### Telegram Setup (via third-party bridge)
If using a custom bridge (e.g. `telegram-opencode`), the bridge runs as a separate Node.js service that connects Telegram to another agent's API. Configure the bridge's `.env` with your bot token and API endpoint, then run `npm start` in the bridge directory.
